  ...
  const fetchPosts = fetch("http://localhost:3000/posts")
    ...
    .then((data) => {
      ...
      if (listGroup.children.length == 0) {
        //if ul hasnt got childs, we create them
        data.sort(function (a, b) {
          return a.id.toString() - b.id.toString();
        });
        data.forEach((post) => {
          const postElement = document.createElement("section");
          assignAttributes(postElement, {
            "data-bs-toggle": "modal",
            "data-bs-target": "#staticBackdrop",
            id: post.id,
            userId: post.userId,
            postOrder: data.indexOf(post),
          });
        ...
        //if the user deleted some posts or modify some post, we delete all li and create all of them from the beggining
        Array.from(document.getElementById("listGroup").children).forEach(
          (son) => {
            son.remove();
          }
        );
